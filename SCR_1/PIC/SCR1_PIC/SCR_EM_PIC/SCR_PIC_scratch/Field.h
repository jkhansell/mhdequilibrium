...
class Field_{
	public:
		...
		/*performs element by element division by another scalar*/
		...
		/*performs element by element division by another field*/
		friend void operator /= (Field_<vec3<T>> &f, const Field_<T> &d) {
		...
		/*performs element by element multiplication by another field*/
		friend Field_<vec3<T>> operator* (const Field_<T> &d, const Field_<vec3<T>> &f) {
			Field_<vec3<T>> res(d.ni,d.nj,d.nk);
			for (int i=0;i<f.ni;i++)
				for (int j=0;j<f.nj;j++)
					for(int k=0;k<f.nk;k++){
						res[i][j][k] = f(i,j,k)*d(i,j,k);
				}
			return res;
		}
