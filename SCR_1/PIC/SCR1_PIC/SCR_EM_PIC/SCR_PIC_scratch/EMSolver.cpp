...
/** converts 1D vector to 3D field*/
Field vec::inflate(dvector &d1, int ni, int nj, int nk)
{
	Field f3(ni,nj,nk);
	for (int i=0;i<f3.ni;i++)
			for (int j=0;j<f3.nj;j++)
...
void EMSolver::buildMatrix()
{
	...
	for (int k=0;k<nz;k++)
		for (int j=0;j<nphi;j++)
			for (int i=0;i<nr;i++)
			{
				double dphi = i*dh[0]*dh[1];
				double idphi = 1.0/dphi;
				double idphi2 = idphi*idphi;
				...
				if (i==0) {A(u,u)=idr;A(u,u+1)=-idr;}
				else if (i==nr-1) {A(u,u)=idr;A(u,u-1)=-idr;}
				else if (j==0) {A(u,u)=idphi;A(u,u+nr)=-idphi;}
				else if (j==nphi-1) {A(u,u)=idphi;A(u,u-nr)=-idphi;}
				else if (k==0) {A(u,u)=idz;A(u,u+nr*nphi)=-idphi;}
				else if (k==nz-1) {A(u,u)=idz;A(u,u-nr*nphi)=-idphi;}
				else {
					//standard internal stencil
					A(u,u-nr*nphi) = idz2; 
					A(u,u-nr) = idphi2;
					A(u,u-1) = idr2;
					A(u,u) = -2.0*(idr2+idphi2);
					A(u,u+1) = idr2;
					A(u,u+nr) = idphi2;
					A(u,u+nr*nphi) = idz2; 
...
void EMSolver::init() {
	Field b3 = -1*(1/Const::EPS_0)*world.rho;
	...
	world.phi = vec::inflate(phi_s, world.nr, world.nphi, world.nz);

	world.E = -1*grad(world.phi, world);
	applyBoundaries();
...
{
	Field3 gf(f.ni,f.nj,f.nk);	//this should be optimized out
	...
	for (int i=0;i<world.nr;i++)
		for (int j=0;j<world.nphi;j++)
			for (int k=0;j<world.nz;j++){
			
				double r = i*dh[0]; 
				/*r component*/
				if (i==0)
					gf[i][j][k] = -(-3*f(i,j,k)+4*f(i+1,j,k)-f(i+2,j,k))/(2*dr);	/*forward*/
				else if (i==world.nr-1)
					gf[i][j][k] = -(f(i-2,j,k)-4*f(i-1,j,k)+3*f(i,j,k))/(2*dr);	/*backward*/
				else
					gf[i][j][k] = -(f(i+1,j,k) - f(i-1,j,k))/(2*dr);	/*central*/

				
				/*phi component*/
				if (j==0)
					gf[i][j][k] = -(-3*f(i,j,k)+4*f(i,j+1,k)-f(i,j+2,k))/(2*r*dphi);	/*forward*/
				else if (j==world.nphi-1)
					gf[i][j][k] = -(f(i,j-2,k)-4*f(i,j-1,k)+3*f(i,j,k))/(2*r*dphi);	/*backward*/
				else
					gf[i][j][k] = -(f(i,j+1,k) - f(i,j-1,k))/(2*r*dphi);	/*central*/

				/*z component*/
				if (k==0)
					gf[i][j][k] = -(-3*f(i,j,k)+4*f(i,j,k+1)-f(i,j,k+2))/(2*dz);	/*forward*/
				else if (k==world.nz-1)
					gf[i][j][k] = -(f(i,j,k-2)-4*f(i,j,k-1)+3*f(i,j,k))/(2*dz);	/*backward*/
				else
					gf[i][j][k] = -(f(i,j,k+1) - f(i,j,k-1))/(2*dz);	/*central*/
...
Field3 EMSolver::curl(Field3 &f, bool inner) {
	...
	if (inner) {ni++;nj++;nk++;}		//outer mesh is larger

	Field3 res(ni,nj,nk);	//this should be optimized out
	double dr = world.getDh()[0];
	double dphi = world.getDh()[1];
	double dz = world.getDh()[2];
	int o = inner?1:0;

	for (int i=0;i<f.ni-1;i++)
		for (int j=0;j<f.nj-1;j++) 
			for (int k=0;j<f.nk-1;k++){
